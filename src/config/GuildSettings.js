export default {
  'But Why Tho': {
    channels: [
      'maribot_commandments'
    ],
    roles: [
      'admin',
      'Bot Commander',
      'member'
    ],
    react: true
  },
  'Bot Testing Grounds': {
    channels: [
      'botcommands'
    ],
    levels: [
      {
        messagesRequired: 0,
        name: 'one'
      },
      {
        messagesRequired: 5,
        name: 'two'
      },
      {
        messagesRequired: 7,
        name: 'three'
      },
      {
        messagesRequired: 9,
        name: 'four',
        max: true
      }
    ],
    levelUpMessage: '<user> has levelled up to <level>',
    maxLvlMessage: 'GG <user>, you\'ve reached MAX LEVEL!',
    welcomeMessage: 'Welcome fellow human \'<user>\' to our testing area.',
    welcomeChannel: '265600449762033664',
    managedRoles: [ 'TestRole1', 'TestRole2' ],
    roleReactions: [ 'robot', 'clock' ],
    roleMessage: 'React :robot: for role 1, and :clock: for role 2.',
    react: true
  },
  'MARI-BOTS': {
    channels: [
      'botcommands'
    ],
    welcomeMessage: '<user> just joined MARI-BOTS! Let\'s have fun together!',
    welcomeChannel: '413407563099668501',
    roles: [
      'Admin Sadmen',
      'MARI TIME',
      'Employee of the Month (Twitch SUB)',
	  'Casuals'
    ],
    levels: [
      {
        messagesRequired: 0,
        name: '0'
      },
      {
        messagesRequired: 5,
        name: '1'
      },
      {
        messagesRequired: 10,
        name: '2'
      },
      {
        messagesRequired: 18,
        name: '3'
      },
      {
        messagesRequired: 30,
        name: '4'
      },
      {
        messagesRequired: 52,
        name: '5'
      },
      {
        messagesRequired: 90,
        name: '6'
      },
      {
        messagesRequired: 160,
        name: '7'
      },
      {
        messagesRequired: 280,
        name: '8'
      },
      {
        messagesRequired: 490,
        name: '9'
      },
      {
        messagesRequired: 850,
        name: '10',
        max: true
      }
    ],
    levelUpMessage: 'CONGRAGULATIONS <user>, YOU\'VE LEVELED UP TO LEVEL <level>!',
    maxLvlMessage: 'GG <user>, YOU\'VE REACHED MAX LEVEL!',
    react: true
  },
  'Magnet\'s Den of Sin': {
    channels: [
      'botcommands'
    ],
    welcomeMessage: 'Welcome to the sin fest, <user>',
    welcomeChannel: '603039520367050764',
    roles: [
      'Mods'
    ],
    levels: [
      {
        messagesRequired: 0,
        name: '0'
      },
      {
        messagesRequired: 5,
        name: '1'
      },
      {
        messagesRequired: 10,
        name: '2'
      },
      {
        messagesRequired: 18,
        name: '3'
      },
      {
        messagesRequired: 30,
        name: '4'
      },
      {
        messagesRequired: 52,
        name: '5'
      },
      {
        messagesRequired: 90,
        name: '6'
      },
      {
        messagesRequired: 160,
        name: '7'
      },
      {
        messagesRequired: 280,
        name: '8'
      },
      {
        messagesRequired: 490,
        name: '9'
      },
      {
        messagesRequired: 850,
        name: '10',
        max: true
      }
    ],
    levelUpMessage: 'CONGRAGULATIONS <user>, YOU\'VE LEVELED UP TO LEVEL <level>!',
    maxLvlMessage: 'GG <user>, YOU\'VE REACHED MAX LEVEL!',
    react: true
  }
}
